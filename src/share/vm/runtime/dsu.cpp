      ...
      if (stale_klass->is_stale_class()) {
        ...
        if (stale_klass->next_version() == NULL) {
          ...
          if (stale_klass->previous_version() != NULL) {
            ...
            DSU_DEBUG(("Enter transforming object via a custom transformer method."));
...
void collect_transformer_arguments(JavaCallArguments &args, Handle inplace_object, Handle phantom_object, Array<u1>* transformer_args, TRAPS) {
  ...
  if (transformer_args != NULL) {
    ...
    DSU_DEBUG(("Read %d transformer arguments.", length));
...
void run_custom_transformer(Handle inplace_object, InstanceKlass* old_phantom_klass, Method* object_transformer, JavaCallArguments &args, TRAPS) {
  if (object_transformer != NULL) {
    ...
    assert(old_phantom_klass->stale_new_class()->is_stale_class(), "temp version must be not NULL");
    assert(old_phantom_klass->stale_new_class()->previous_version() != NULL, "temp version must be not NULL");
...
void Javelus::parse_old_field_annotation(InstanceKlass* the_class,
  InstanceKlass* transformer, Array<u1>* annotations, Array<u1>* &result, TRAPS) {
    ...
    if (DSU_TRACE_ENABLED(4096)) {
      DSU_DEBUG(("Original annotation: "));
      for(int i = 0; i < annotations->length(); i++) {
        DSU_DEBUG(("%0x ", annotations->at(i)));
      }
      tty->cr();
    }

    DSU_TRACE(4096, ("Parse annotations, number of parameters %d, attribute length is %d", num_parameters, attribute_length));
    if (num_parameters > 1) {
      // set up annotation
      ...
      while(cursor < attribute_length) {
        ...
        DSU_TRACE(4096, ("Parse annotations, cursor is %d, transformer field index is %d, num annotations is %d",
...
void Javelus::increment_system_rn() {
  if (_latest_rn >= Javelus::MAX_REVISION_NUMBER) {
    DSU_WARN(("Revision Number Overflow!!!"));
  }
  _latest_rn ++;
}
